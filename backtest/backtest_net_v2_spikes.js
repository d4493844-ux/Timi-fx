...
// Trend/momentum signal for forex, crypto, jump indices (these trend, unlike
// BOOM/CRASH which spike). EMA alignment + RSI confirmation. This is a TA
// BASELINE — the live engine uses ML ensembles for these, so treat results as
// a floor, not the live expectation.
function trendSignal(candles) {
  const closes=candles.map(c=>parseFloat(c.close));
  const price=closes[closes.length-1];
  const ema8=calcEMA(closes,8), ema21=calcEMA(closes,21);
  const ema50=calcEMA(closes.slice(-60),50);
  ...
  let action=null, conf=0;
  // trade with the trend, confirmed by RSI not being exhausted
  if (ema8>ema21 && ema21>ema50 && rsi>50 && rsi<70) { action="BUY";  conf=65+(rsi-50); }
  if (ema8<ema21 && ema21<ema50 && rsi<50 && rsi>30) { action="SELL"; conf=65+(50-rsi); }
  if (!action) return null;
  return { action, confidence:Math.min(90,conf), atr,
           tp_mult:1.5, sl_mult:1.5, window:5 };
}
...
function getSignal(candles, candles5m, symbol) {
  ...
  // forex / crypto / jump → trend baseline (live uses ML)
  return trendSignal(candles);
}
...
async function backtest(symbol, granularity=60, count=1500, minConf=60, label="") {
  ...
  for (let i=60; i<candles.length-25; i++) {
    ...
    // REALISTIC SPIKE FILL: when a stop is hit, the position fills at the
    // candle's actual extreme (low for a long, high for a short), NOT at the
    // neat stop level. On BOOM/CRASH a spike gaps straight through the stop,
    // so the real loss can be many ATR — this is the tail risk a naive
    // backtest hides behind a high win rate.
    let won=false, resolved=false, exitPrice=null;
    for (let j=i+1;j<=i+W;j++) {
      const h=parseFloat(candles[j].high), l=parseFloat(candles[j].low);
      if (sig.action==="BUY")  {
        if(l<=sl){won=false;resolved=true;exitPrice=Math.min(sl,l);break;}   // fill at actual low
        if(h>=tp){won=true; resolved=true;exitPrice=tp;break;}
      } else {
        if(h>=sl){won=false;resolved=true;exitPrice=Math.max(sl,h);break;}   // fill at actual high
        if(l<=tp){won=true; resolved=true;exitPrice=tp;break;}
      }
    }
    if (!resolved) { const fc=parseFloat(candles[i+W].close); won=sig.action==="BUY"?fc>entry:fc<entry; exitPrice=fc; }

    // Realized R-multiple from the ACTUAL exit price (captures spike overshoot)
    const riskDist = atr*sig.sl_mult;
    const moved = sig.action==="BUY" ? (exitPrice-entry) : (entry-exitPrice);
    const rMultiple = riskDist>0 ? moved/riskDist : 0;   // +ve = profit in R units

    // GROSS pnl
    let gPnl, nPnl;
    if (cls==="vix") {
      // CALL/PUT: payout-based, fixed win/loss, no separate cost
      gPnl = won ? STAKE*0.95 : -STAKE;
      nPnl = won ? STAKE*(VIX_PAYOUT-1) : -STAKE;
    } else {
      // MULT: PnL scales with the REALIZED move (rMultiple), not a fixed amount.
      // A spike that gaps through the stop produces rMultiple well below -1,
      // i.e. a loss larger than one risk unit. Deriv caps MULT loss at the
      // stake, so we floor the loss at -STAKE (the contract's real limit) —
      // but the point is the rare loss is now BIG, not a tidy -1.
      const rawR = rMultiple;                 // realized profit in risk units
      // map R to $: win side uses the TP payout, loss side uses actual overshoot
      let raw = won ? STAKE*0.95 : STAKE * rawR;   // rawR is negative on a loss
      raw = Math.max(raw, -STAKE);            // MULT can't lose more than stake
      gPnl = raw;
      nPnl = raw - cost;                       // real commission every trade
    }
    ...
    if (nPnl < worstTrade) worstTrade = nPnl;
  ...
  console.log(`   Worst trade:     $${worstTrade.toFixed(3)}  ${worstTrade<=-STAKE?"⚠️ full-stake spike loss":""}`);
...
async function main() {
  ...
  const symbols=[
    ...
    {sym:"BOOM300N",  gran:60, label:"spike"},
    {sym:"CRASH1000", gran:60, label:"spike"},
    {sym:"CRASH500",  gran:60, label:"spike"},
    {sym:"CRASH300N", gran:60, label:"spike"},
    // ── JUMP indices ──
    {sym:"JD10",      gran:60, label:"jump"},
    {sym:"JD25",      gran:60, label:"jump"},
    {sym:"JD50",      gran:60, label:"jump"},
    {sym:"JD75",      gran:60, label:"jump"},
    {sym:"JD100",     gran:60, label:"jump"},
    // ── VIX (R_) ──
    {sym:"R_10",      gran:60, label:"vix"},
    {sym:"R_25",      gran:60, label:"vix"},
    {sym:"R_50",      gran:60, label:"vix"},
    {sym:"R_75",      gran:60, label:"vix"},
    {sym:"R_100",     gran:60, label:"vix"},
    // ── FOREX (majors + crosses + metals) ──
    {sym:"frxEURUSD", gran:60, label:"forex"},
    {sym:"frxGBPUSD", gran:60, label:"forex"},
    {sym:"frxUSDJPY", gran:60, label:"forex"},
    {sym:"frxAUDUSD", gran:60, label:"forex"},
    {sym:"frxUSDCAD", gran:60, label:"forex"},
    {sym:"frxEURGBP", gran:60, label:"forex"},
    {sym:"frxEURJPY", gran:60, label:"forex"},
    {sym:"frxGBPJPY", gran:60, label:"forex"},
    {sym:"frxAUDJPY", gran:60, label:"forex"},
    {sym:"frxCADJPY", gran:60, label:"forex"},
    {sym:"frxNZDJPY", gran:60, label:"forex"},
    {sym:"frxXAUUSD", gran:60, label:"forex"},   // gold
    {sym:"frxXAGUSD", gran:60, label:"forex"},   // silver
    // ── CRYPTO ──
    {sym:"cryBTCUSD", gran:60, label:"crypto"},
    {sym:"cryETHUSD", gran:60, label:"crypto"},
  ];
  const results=[];
  for (const {sym,gran,label} of symbols) {
    ...
    await new Promise(r=>setTimeout(r, 800)); // throttle: free app_id is rate-limited
  }
