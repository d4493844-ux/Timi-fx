...
// run a config with a chosen entry style, RR, hold — optionally logging trades
function run(candles,mult,RR,maxHold,entryStyle,log){
  ...
  for(let i=60;i<candles.length-maxHold-1;i++){
    ...
    bal+=pnl; if(won)w++;else l++;
    if(log) log.push({i,symbol:"JD100",action,won,net_pnl:+pnl.toFixed(4)});
  }
...
async function main(){
  ...
  if(best){
    ...
    // write trade log of the best config across all windows for significance testing
    const log=[];
    for(const w of windows){ if(!w)continue; run(w.candles,best.mult,best.RR,best.hold,best.entry,log); }
    require("fs").writeFileSync("jd100_hunt_trades.json", JSON.stringify(log,null,2));
    console.log(`\n   💾 ${log.length} trades → jd100_hunt_trades.json`);
    console.log("   ⚠️ CRITICAL NEXT STEP: node backtest/significance_suite.js jd100_hunt_trades.json");
    console.log("      Watch the SKEW. ~+1 = broad real edge. +4/+5 = riding a few lucky jumps (mirage).");
  } else {
