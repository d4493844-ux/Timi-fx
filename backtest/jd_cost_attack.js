...
const COMM=0.0015, SPREAD=0.0;   // JD synthetics: no bid/ask spread, commission only (like BOOM/CRASH)
...
function runConfig(candles, RR, mult, maxHold, symbol){
  const stake=1, cost=entryCost(stake,mult);
  let wins=0,losses=0,bal=0;
  const rets=[]; const tradeLog=[];
  for(let i=60;i<candles.length-maxHold-1;i++){
    ...
    tradeLog.push({i,symbol:symbol||"?",action:sig.action,won,net_pnl:+pnl.toFixed(4)});
  }
  ...
  return {total,wr:+(wr*100).toFixed(1),be:+(be*100).toFixed(1),
    net:+bal.toFixed(2),sharpe:+sharpe.toFixed(2),beats:wr>=be,tradeLog};
}

async function main(){
  console.log("🔬 TIMI JD (JUMP) COST-ATTACK TEST\n");
  console.log("JD direction is ~50%. Instead of beating that, we LOWER the");
  console.log("breakeven via reward:risk + multiplier, so 50% becomes profitable.\n");

  const pairs=["JD10","JD25","JD50","JD75","JD100"];
  ...
  for(const pair of pairs){
    ...
    for(const cfg of configs){
      const r=runConfig(candles,cfg.RR,cfg.mult,cfg.maxHold,pair);
      if(!r) continue;
      if(r.beats && r.net>0){
        winners.push({pair,...cfg,total:r.total,wr:r.wr,be:r.be,net:r.net,sharpe:r.sharpe});
        if(!best||r.net>best.net) best={...cfg,...r};
      }
    }
    const sampleRR2 = runConfig(candles,2.0,30,16,pair);
    ...
    const sampleRR3 = runConfig(candles,3.0,30,24,pair);
    ...
    if(best){
      console.log(`   ⭐ BEST:   RR${best.RR} x${best.mult}: WR ${best.wr}% (need ${best.be}%) net $${best.net} Sharpe ${best.sharpe} 🟢`);
      // collect this pair's best-config trades, tagged with pair so suite groups by symbol
      for(const t of best.tradeLog) allBestTrades.push(t);
    }
    else console.log(`   no profitable config found`);
  }

  // write trade log for the significance suite to interrogate
  require("fs").writeFileSync("jd_attack_trades.json", JSON.stringify(allBestTrades,null,2));
  console.log(`\n💾 ${allBestTrades.length} trades (best config per pair) → jd_attack_trades.json`);
