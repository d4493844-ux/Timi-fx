...
def train_symbol(symbol):
    ...
    # Meta-labeling
    print("  🧠 Meta-labeling filter...")
    all_probs=ens_predict(best_models,X)
    ...
    result={"symbol":symbol,"instrument_class":cls,"cv_accuracy":round(float(avg),4),
        ...
        "deploy":best_wr>=0.55 and best_cnt>=20,"trained_at":str(pd.Timestamp.now()),
    ...
    print(f"  {'✅ DEPLOY' if result['deploy'] else '❌ SKIP'} | {best_wr*100:.1f}% WR | {best_cnt} trades")
    return result
...
def main():
    ...
    targets=["frxEURUSD","frxGBPUSD","frxUSDJPY","frxGBPJPY",
             "frxXAUUSD","cryBTCUSD","cryETHUSD","R_50","R_25","R_100"]
